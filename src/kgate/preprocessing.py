...
def load_knowledge_graph(pickle_filename: Path) -> Tuple[KnowledgeGraph,KnowledgeGraph,KnowledgeGraph]:
    ...
    The pickle file must contain only three objects in the correct order: the training, validation and test dataset.
    WARNING: This method unsafely loads pickle files, which may lead to malicious code execution. Only use it with trusted files,
    preferrably generated by a previous run of KGATE.
